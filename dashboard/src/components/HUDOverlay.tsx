  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10">
      ...
      <motion.div
        ...
        className="pointer-events-auto absolute right-5 top-5 flex flex-col items-end gap-1.5"
      >
        <div className="hud-panel flex items-center gap-2 px-3 py-1.5">
          ...
          <span className={`text-[10px] tracking-wider ${connected ? 'text-white/60' : 'text-red-400/60'}`}>
        ...
        {[
          ...
          <button
            ...
            className="hud-panel flex items-center gap-2 px-3 py-1.5 transition-all hover:bg-white/[0.06] group"
          >
            <item.icon className="h-3 w-3 text-white/30 group-hover:text-white/60 transition-colors" />
            <span className="text-[10px] text-white/30 group-hover:text-white/60 tracking-wider transition-colors">
              {item.label.toUpperCase()}
            </span>
            <ChevronRight className="h-2.5 w-2.5 text-white/15 group-hover:text-white/40 transition-colors" />
          </button>
        ))}
      </motion.div>
      ...
      >
        <div className="flex items-center gap-2 mb-2 border-b border-white/[0.04] pb-2">
          ...
          <span className="text-[10px] text-white/50 tracking-[0.15em] uppercase">
            Live Feed
          </span>
        ...
        <div ref={feedRef} className="max-h-44 overflow-y-auto scrollbar-thin space-y-0.5">
            ...
            {recentEvents.map((event) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-2 py-1 text-[10px] border-b border-white/[0.02]"
                >
                  <span className="text-white/20 w-14 shrink-0 tabular-nums">{time}</span>
          ...
          {recentEvents.length === 0 && (
            <div className="text-center py-4 text-white/15 text-[10px]">
              Awaiting telemetry...
            </div>
          )}
        </div>
      ...
      <motion.div
        ...
        className="pointer-events-auto absolute bottom-5 right-5 hud-panel w-48"
      >
        <div className="flex items-center gap-2 mb-2 border-b border-white/[0.04] pb-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-xl font-light text-white/80 tabular-nums">
        ...
        {liveEvents.find((e) => e.decision === 'BLOCK') && (
          <div className="mt-2 pt-2 border-t border-white/[0.04]">
            <span className="text-[8px] text-white/20 block mb-0.5 uppercase tracking-wider">Latest</span>
...
const StatItem: React.FC<StatItemProps> = ({ icon: Icon, label, value, prefix, decimals, accent }) => (
  <div className="flex items-center gap-2 py-0.5">
    <Icon className={`h-3 w-3 ${accent ? 'text-red-400/40' : 'text-white/20'}`} />
    <div className="flex flex-col">
      <span className="text-[8px] text-white/25 uppercase tracking-wider">{label}</span>
      <span className={`text-sm font-light tabular-nums ${accent ? 'text-red-400/70' : 'text-white/70'}`}>
        {prefix}
