...
const DANGEROUS_PATTERNS: DangerousPattern[] = [
  ...
  {
    pattern: /rm\s+.*-[rRf]*\s+--no-preserve-root/,
    reason: 'Explicit root deletion bypass',
    severity: 'critical',
  },
  {
  ...
  { pattern: /eval\s*\(?\s*["'`]/, reason: 'Dynamic code evaluation', severity: 'high' },
  {
    ...
    reason: 'Setting world-executable permissions',
    severity: 'medium',
  },
  {
    pattern: /nc\s+-[elp]/,
    reason: 'Netcat listener (potential reverse shell)',
    severity: 'critical',
  },
  { pattern: />\s*\/etc\//, reason: 'Direct write to /etc/', severity: 'high' },
  {
    pattern: /python[3]?\s+-c\s+['"].*(?:import\s+(?:os|subprocess|socket))/,
    reason: 'Python inline with dangerous imports',
    severity: 'high',
  },
  {
    pattern: /base64\s+(?:-d|--decode)\s*\|/,
    reason: 'Decoding base64 into pipe',
    severity: 'medium',
  },
  {
    pattern: /\$\(.*base64\s+(?:-d|--decode)/,
    reason: 'Base64 decode in subshell expansion',
    severity: 'critical',
  },
  {
    pattern: /echo\s+[A-Za-z0-9+/=]+\s*\|\s*base64\s+(?:-d|--decode)/,
    reason: 'Echo piped to base64 decode (possible encoded command)',
    severity: 'high',
  },
];
