...
export function formatWeeklyMarkdown(report: WeeklyReport): string {
  ...
  if (report.summary.piiLeaks > 0) lines.push(`  - ${report.summary.piiLeaks} PII leak(s) prevented`);
  if (report.summary.injectionAttempts > 0) lines.push(`  - ${report.summary.injectionAttempts} injection attempt(s) blocked`);
  if (report.summary.dangerousCommands > 0) lines.push(`  - ${report.summary.dangerousCommands} dangerous command(s) stopped`);
  if (report.summary.networkViolations > 0) lines.push(`  - ${report.summary.networkViolations} network violation(s) caught`);
  ...
  lines.push('---');
  lines.push('*Generated by [Puffer](https://github.com/puffer-fish/puffer) — The autonomous immune system for AI agents*');
