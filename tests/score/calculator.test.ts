...
import { calculateScore, type AuditStats } from '@puffer/score';
...
describe('Puffer Score Calculator', () => {
  ...
  it('gives higher policy score for enforce vs monitor mode', () => {
    const enforceScore = calculateScore(
      makeConfig({ mode: 'enforce' }),
      makeDiscovery(),
      emptyStats,
    );
    const monitorScore = calculateScore(
      makeConfig({ mode: 'monitor' }),
      makeDiscovery(),
      emptyStats,
    );
    expect(enforceScore.breakdown.policy.score).toBeGreaterThan(
      monitorScore.breakdown.policy.score,
    );
  });
  ...
  it('gives full monitoring with webhook configured', () => {
    const config = makeConfig({
      alerts: { desktop: true, webhook: 'https://hooks.slack.com/test' },
    });
  ...
  it('assigns correct grades', () => {
    ...
    const config = makeConfig({
      mode: 'paranoid',
      alerts: { desktop: true, webhook: 'https://test' },
    });
  ...
  it('never exceeds 100', () => {
    const agents = Array(10)
      .fill(null)
      .map((_, i) => makeAgent(`agent-${i}`, 'protected'));
    const config = makeConfig({
      mode: 'paranoid',
      alerts: { desktop: true, webhook: 'https://test' },
    });
